...
Runs the specified step (method named step_<step_name>),
if step has failed, puts @$ into step summary.
...
sub run_step {
    ...
    eval {
        my $method_name = "step_$step_name";
        $self->$method_name();
        1;
    } or do {
        my $err = $@;
...
sub get_config_values {
    ...
    if ($retval->{debugLevel}) {
        $self->logger->level($retval->{debugLevel});
        $self->logger->debug('Debug is enabled');
        $self->logger->trace('Trace is enabled');
    }
